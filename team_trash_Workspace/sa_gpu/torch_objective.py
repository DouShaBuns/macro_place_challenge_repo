...
class TorchProxyCostEvaluator:
    def __init__(
        ...
        self.has_fixed = bool(self.benchmark.macro_fixed.any())
        ...
        if self.num_hard > 1:
            self.hard_pair_i, self.hard_pair_j = torch.triu_indices(
                self.num_hard,
                self.num_hard,
                offset=1,
                device=self.device,
            ...
            self.hard_pair_i = torch.empty(0, dtype=torch.long, device=self.device)
            self.hard_pair_j = torch.empty(0, dtype=torch.long, device=self.device)
        self.grid_row_ids = torch.arange(self.grid_rows, device=self.device).repeat_interleave(self.grid_cols)
        self.grid_col_ids = torch.arange(self.grid_cols, device=self.device).repeat(self.grid_rows)

    def evaluate_batch(self, placements: torch.Tensor, check_overlap: bool = True) -> CostBreakdown:
        if placements.dim() == 2:
        ...
        if check_overlap:
            ...
            overlap_count = torch.zeros(placements.shape[0], dtype=torch.long, device=self.device)
            total_overlap = torch.zeros(placements.shape[0], dtype=torch.float32, device=self.device)
            max_overlap = torch.zeros(placements.shape[0], dtype=torch.float32, device=self.device)
    ...
    def _overlap(self, placements):
        ...
        i = self.hard_pair_i
        j = self.hard_pair_j
        ox = (torch.minimum(right[:, i], right[:, j]) - torch.maximum(left[:, i], left[:, j])).clamp_min(0)
        oy = (torch.minimum(top[:, i], top[:, j]) - torch.maximum(bottom[:, i], bottom[:, j])).clamp_min(0)
        pair_area = ox * oy
    ...
    def _macro_blockage(self, placements, grid_v_routes: float, grid_h_routes: float):
        ...
        v_keep = intersects & ~(partial_v.unsqueeze(2) & (self.grid_row_ids.view(1, 1, -1) == ur_row.unsqueeze(2)))
        h_keep = intersects & ~(partial_h.unsqueeze(2) & (self.grid_col_ids.view(1, 1, -1) == ur_col.unsqueeze(2)))
        v = (
